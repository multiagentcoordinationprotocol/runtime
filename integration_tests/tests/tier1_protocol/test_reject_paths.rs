...
async fn decision_commitment_from_non_initiator_rejected() {
    ...
    let ack = send_as(
        ...
        envelope(
            ...
            &sid,
            voter,
            commitment_payload("c1", "option-A", "team", "unauthorized", true),
        ),
    )
    .await
    .unwrap();
...
async fn proposal_commitment_without_accept_rejected() {
    ...
    let ack = send_as(
        ...
        envelope(
            ...
            &sid,
            buyer,
            commitment_payload("c1", "accept-offer", "negotiation", "premature", true),
        ),
    )
    .await
    .unwrap();
...
async fn handoff_accept_without_offer_rejected() {
    ...
    assert!(
        !ack.ok,
        "HandoffAccept without prior HandoffOffer must be rejected"
    );
}
...
async fn quorum_commitment_before_quorum_reached_rejected() {
    ...
    let ack = send_as(
        ...
        envelope(
            ...
            &sid,
            requester,
            commitment_payload("c1", "deploy", "ops", "premature", true),
        ),
    )
    .await
    .unwrap();
...
async fn multi_round_commitment_before_convergence_rejected() {
    ...
    let ack = send_as(
        ...
        envelope(
            ...
            &sid,
            agent_a,
            commitment_payload("c1", "premature", "group", "not converged", true),
        ),
    )
    .await
    .unwrap();
