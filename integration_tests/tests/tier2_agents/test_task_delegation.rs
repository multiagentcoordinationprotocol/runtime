...
async fn rig_tools_drive_task_delegation() {
    ...
    planner_tools.add_tool(StartSessionTool { client: planner_client.clone(), agent_id: "agent://planner".into() });
    planner_tools.add_tool(TaskRequestTool { client: planner_client.clone(), agent_id: "agent://planner".into() });
    planner_tools.add_tool(CommitTool { client: planner_client.clone(), agent_id: "agent://planner".into(), mode: MODE_TASK.into() });

    let mut worker_tools = ToolSet::default();
    worker_tools.add_tool(TaskAcceptTool { client: worker_client.clone(), agent_id: "agent://worker".into() });
    worker_tools.add_tool(TaskCompleteTool { client: worker_client.clone(), agent_id: "agent://worker".into() });

    // Planner starts session
    let r = planner_tools.call("macp_start_session", serde_json::json!({
    ...
    let r = planner_tools.call("macp_task_request", serde_json::json!({
    ...
    // Worker accepts
    let r = worker_tools.call("macp_task_accept", serde_json::json!({
        "session_id": &sid, "task_id": "t1"
    }).to_string()).await.unwrap();
    assert!(serde_json::from_str::<serde_json::Value>(&r).unwrap()["ok"].as_bool().unwrap());

    // Worker completes
    let r = worker_tools.call("macp_task_complete", serde_json::json!({
        "session_id": &sid, "task_id": "t1", "summary": "Q4 metrics show 20% growth"
    }).to_string()).await.unwrap();
    assert!(serde_json::from_str::<serde_json::Value>(&r).unwrap()["ok"].as_bool().unwrap());

    // Planner commits
    let r = planner_tools.call("macp_commit", serde_json::json!({
