...
async fn rig_tools_drive_quorum_approval() {
    ...
    req_tools.add_tool(StartSessionTool {
        client: req_client.clone(),
        agent_id: "agent://requester".into(),
    });
    req_tools.add_tool(ApprovalRequestTool {
        client: req_client.clone(),
        agent_id: "agent://requester".into(),
    });
    req_tools.add_tool(CommitTool {
        client: req_client.clone(),
        agent_id: "agent://requester".into(),
        mode: MODE_QUORUM.into(),
    });

    let mut a1_tools = ToolSet::default();
    a1_tools.add_tool(ApproveTool {
        client: a1_client.clone(),
        agent_id: "agent://approver1".into(),
    });

    let mut a2_tools = ToolSet::default();
    a2_tools.add_tool(ApproveTool {
        client: a2_client.clone(),
        agent_id: "agent://approver2".into(),
    });
