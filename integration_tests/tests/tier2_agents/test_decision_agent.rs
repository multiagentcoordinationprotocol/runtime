...
use macp_integration_tests::macp_tools::{self, decision::*, commit::*, query::*, session_start::*};
...
async fn rig_tools_drive_decision_lifecycle() {
    ...
    coord_tools.add_tool(StartSessionTool { client: coord_client.clone(), agent_id: "agent://coord".into() });
    coord_tools.add_tool(ProposeTool { client: coord_client.clone(), agent_id: "agent://coord".into() });
    coord_tools.add_tool(CommitTool { client: coord_client.clone(), agent_id: "agent://coord".into(), mode: MODE_DECISION.into() });
    coord_tools.add_tool(GetSessionTool { client: coord_client.clone(), agent_id: "agent://coord".into() });

    // Build voter's toolset
    let mut voter_tools = ToolSet::default();
    voter_tools.add_tool(VoteTool { client: voter_client.clone(), agent_id: "agent://voter".into() });
    ...
    let result = coord_tools.call("macp_start_session", serde_json::json!({
        "mode": MODE_DECISION,
        "session_id": &sid,
    ...
    let result = coord_tools.call("macp_propose", serde_json::json!({
        "session_id": &sid,
        "proposal_id": "p1",
        "option": "deploy-v2",
    ...
    // Step 3: Voter votes
    let result = voter_tools.call("macp_vote", serde_json::json!({
        "session_id": &sid,
        "proposal_id": "p1",
        "vote": "approve",
    ...
    let result = coord_tools.call("macp_commit", serde_json::json!({
        "session_id": &sid,
        "action": "deploy-v2",
    ...
    let result = coord_tools.call("macp_get_session", serde_json::json!({
        "session_id": &sid
    }).to_string()).await.unwrap();
    let result: serde_json::Value = serde_json::from_str(&result).unwrap();
