...
async fn rig_tools_drive_proposal_negotiation() {
    ...
    buyer_tools.add_tool(StartSessionTool { client: buyer_client.clone(), agent_id: "agent://buyer".into() });
    buyer_tools.add_tool(AcceptProposalTool { client: buyer_client.clone(), agent_id: "agent://buyer".into() });
    buyer_tools.add_tool(CommitTool { client: buyer_client.clone(), agent_id: "agent://buyer".into(), mode: MODE_PROPOSAL.into() });

    let mut seller_tools = ToolSet::default();
    seller_tools.add_tool(SubmitProposalTool { client: seller_client.clone(), agent_id: "agent://seller".into() });
    seller_tools.add_tool(AcceptProposalTool { client: seller_client.clone(), agent_id: "agent://seller".into() });

    // Buyer starts session
    let r = buyer_tools.call("macp_start_session", serde_json::json!({
    ...
    // Seller proposes
    let r = seller_tools.call("macp_submit_proposal", serde_json::json!({
    ...
    // Both accept
    let r = seller_tools.call("macp_accept_proposal", serde_json::json!({
        "session_id": &sid, "proposal_id": "prop-1", "reason": "fair price"
    }).to_string()).await.unwrap();
    assert!(serde_json::from_str::<serde_json::Value>(&r).unwrap()["ok"].as_bool().unwrap());

    let r = buyer_tools.call("macp_accept_proposal", serde_json::json!({
        "session_id": &sid, "proposal_id": "prop-1", "reason": "agreed"
    }).to_string()).await.unwrap();
    assert!(serde_json::from_str::<serde_json::Value>(&r).unwrap()["ok"].as_bool().unwrap());

    // Buyer commits
    let r = buyer_tools.call("macp_commit", serde_json::json!({
